...
void generation(Tlong mas[], int &length, Tlong a, const Tlong &b)
{
    length=0;
    while(a.cmp_with_zero())
    {
        mas[1000-(++length)]=a%b;
        a=a/b;
    }
...
void out_mas(Tlong mas[], int length)
{
    for (int i=1000-length; i<1000; ++i)
        if(mas[i]<10) cout<<mas[i];
        else cout<<'['<<mas[i]<<']';
}

Tlong num_in_base[1000];
int main()
{
    ...
    cin>>a>>b;
    int length_mas;
    generation(num_in_base, length_mas, a, b);
    out_mas(num_in_base, length_mas);
    return 0;
}
