...
void inp_arrays(int mas_dilnik[], int mas_exp[], int &n_dilniks, Tlong n)
{
    n_dilniks=0;
    int dilnik=2;
    while(n!=1)
    {
        while(n%dilnik!=0) ++dilnik;
        while(n%dilnik==0)
        {
            n=n/dilnik;
            mas_dilnik[n_dilniks]=dilnik;
            ++mas_exp[n_dilniks];
        }
        ++n_dilniks;
    }
...
void print_count_symbols(int count, char symbol)
{
    for(int i=0; i<count; ++i) cout<<symbol;
}

int mas_dilnik[1000], mas_exp[1000];
int main()
{
    ...
    int n_dilniks;
    cin>>n;
    inp_arrays(mas_dilnik, mas_exp, n_dilniks, n);
    for(int i=0; i<n_dilniks; ++i)
    {
        print_count_symbols(len_n(mas_dilnik[i]), ' ');
        if(mas_exp[i]>1) cout<<mas_exp[i];
        if(i<n_dilniks-1) cout<<' ';
    }
    cout<<'\n';
    for(int i=0; i<n_dilniks; ++i)
    {
        cout<<mas_dilnik[i];
        if(mas_exp[i]>1) print_count_symbols(len_n(mas_exp[i]), ' ');
        if(i<n_dilniks-1) cout<<'*';
    }
    return 0;
}
