...
void full_mas(Tlong mas_teams[], int n_teams, int n_competition)
{
    int n;
    for (int i=0; i<n_teams; ++i)
    {
        mas_teams[i]=Tlong (1);
        for (int j=0; j<n_competition; ++j)
        {
            cin>>n;
            mas_teams[i]*=n;
        }
...
int ind_max(Tlong mas_teams[], int n_teams)
{
    int etalon=0;
    for(int i=1; i<n_teams; ++i)
        if(mas_teams[i].cmp(mas_teams[etalon]) == 1) etalon=i;
    return etalon;
}
...
int main()
{
    int n_teams, n_competition;
    cin>>n_teams>>n_competition;
    full_mas(mas_teams, n_teams, n_competition);
    cout<<ind_max(mas_teams, n_teams)+1;
    return 0;
}