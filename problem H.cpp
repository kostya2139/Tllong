...
void fulling_mas(int mas_cnt_days[], int mas[], int n)
{
    for(int i=0; i<n; ++i)
    {
        int j=i;
...
int mas_permutaion[15000], mas_cnt_days[15000] {};
int main()
{
    ...
    cout.tie(0);
    int n;
    inp_mas(mas_permutaion, n);
    fulling_mas(mas_cnt_days, mas_permutaion, n);
    Tlong ans(1);
    for(int i=0; i<n; ++i)
        if(mas_cnt_days[i]>1) ans=nsk(ans, mas_cnt_days[i]);
    cout<<ans;
    return 0;
}
